...
client.on(Events.GuildMemberRemove, async (member: any) => {
  ...
  await leaveDB
    .prepare(
      `INSERT OR IGNORE INTO leave(id, user, guild, date) VALUES (?, ?, ?, ?)`
    )
...
client.on("guildMemberAdd", async (member: any) => {
  ...
  if (member.guild.id === "929352993655124000") {
    ...
    const role = member.guild.roles.cache.find(
      (role: any) => role.id === "929352993655124002"
    );
...
player.events.on("error", (queue: any, error: any) => {
  const embed = new EmbedBuilder()
...
player.events.on("playerError", (queue: any, error: any) => {
  const embed = new EmbedBuilder()
    .setTitle("An error occurred while playing")
    .setDescription(`Reason: \`${error.message}\``)
    .setColor(Colors.Red);
...
player.events.on("playerSkip", (queue: any, track: any) => {
...
player.events.on("audioTrackAdd", (queue: any, track: any) => {
  const embed = new EmbedBuilder()
    .setAuthor({
...
player.events.on("audioTracksAdd", (queue: any, tracks: any) => {
  const embed = new EmbedBuilder()
    ...
    if (dropMessage == true) {
      ...
      mongoclient
        ...
        .then(async function (doc: any) {
          try {
            let oldCash = doc.cash;
            let newCash = oldCash + randomMoney;
            mongoclient
              .db("Aylani")
              .collection(`${message.guild.id}Currency`)
