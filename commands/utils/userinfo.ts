...
export default {
  ...
  async execute(client: any, interaction: any) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setTimestamp()
      .addFields(
        { name: "Joined at:", value: `${moment(member.joinedAt).format("dddd, MMMM Do YYYY, HH:mm:ss")}`, inline: true },
        { name: "Created at:", value: `${moment(interaction.author.createdAt).format("dddd, MMMM Do YYYY, HH:mm:ss")}`, inline: true },
        { name: "Permissions:", value: `${permissions.join(", ")} `, inline: true },
        {
          name: `Roles [${member.roles.cache
            .filter((r: any) => r.id !== interaction.guild.id)
            .map((roles: any) => `\`${roles.name}\``).length
            }]`,
          value: `${member.roles.cache
            .filter((r: any) => r.id !== interaction.guild.id)
            .map((roles: any) => `<@&${roles.id}>`)
            .join(" **|** ") || "No Roles"
            }`,
          inline: true
        },
