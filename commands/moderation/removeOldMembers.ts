...
export default {
  ...
  async execute(client: any, interaction: any, mongoclient: any, queue: any) {
    ...
    try {
      ...
      // MONGO DB
      interaction.guild.members.fetch().then((members: any) => {
        const guildUserIDs = members.map((member: any) => member.user.id);
        mongoclient
          .db("Aylani")
          .collection(`${interaction.guild.id}${system}`)
          .find()
          .toArray((err: any, rows: any) => {
            if (err) throw err;
            ...
            rows.forEach((row: any) => {
              if (!guildUserIDs.includes(row.id)) {
                mongoclient
                  .db("Aylani")
                  .collection(`${interaction.guild.id}${system}`)
                  .deleteOne({ id: row.id }, function (err: any, obj: any) {
                    if (err) throw err;
                  });
              }
            });
